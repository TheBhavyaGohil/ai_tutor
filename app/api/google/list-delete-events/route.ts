...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ success: true, deletedCount, events });
  } catch (error: any) {
    console.error("Google List/Delete Events Error:", error);
    return NextResponse.json({ error: error.message || "Failed to list/delete events" }, { status: 500 });
  }
}
